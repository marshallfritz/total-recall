#!/usr/bin/env bash
# sweet-dreams-runner.sh — Observable wrapper for the Sweet Dreams agent
#
# Runs the Sweet Dreams cron payload as a shell-supervised job with:
#   - Persistent tailable log (logs/sweet-dreams-run.log)
...
#
# Usage:
#   OPENCLAW_WORKSPACE=/path/to/workspace bash sweet-dreams-runner.sh
#
# Tail during a run:
#   tail -f ~/.openclaw/workspace/logs/sweet-dreams-run.log

set -euo pipefail

OPENCLAW_WORKSPACE="${OPENCLAW_WORKSPACE:-$HOME/.openclaw/workspace}"
SKILL_DIR="$OPENCLAW_WORKSPACE/skills/total-recall"
LOG_FILE="$OPENCLAW_WORKSPACE/logs/sweet-dreams-run.log"
DREAM_PROMPT="$SKILL_DIR/prompts/sweet-dreams-prompt.md"
...
write_run_header() {
  log_raw ""
  log_raw "════════════════════════════════════════════════════════════"
  log_raw "  Sweet Dreams Run — $RUN_ID"
  log_raw "  Started : $(ts)"
...
write_run_footer() {
  ...
  local history_file="$OPENCLAW_WORKSPACE/logs/sweet-dreams-history.csv"
...
preflight_size_check() {
  ...
  if [ "$bytes" -gt "$TR_OBS_CEILING_BYTES" ]; then
    log WARN "❌ observations.md OVER ceiling (${bytes}B > ${TR_OBS_CEILING_BYTES}B) — Sweet Dreams may time out"
...
# ── Sweet Dreams script hooks ───────────────────────────────────────────────────
# We watch the script subcommands as they're called and emit stage markers.
# Since the Sweet Dreams runs as an LLM agent (not shell), we can't intercept
...
watch_artifacts() {
  ...
  local metrics_dir="$OPENCLAW_WORKSPACE/research/sweet-dreams-metrics/daily"
...
# The Sweet Dreams runs as an isolated LLM agent. We can't join its process —
...
poll_for_completion() {
  ...
  local obs="$OPENCLAW_WORKSPACE/memory/observations.md"
  local backup="$OPENCLAW_WORKSPACE/memory/.dream-backups/observations.pre-dream.md"
  local metrics_file="$OPENCLAW_WORKSPACE/research/sweet-dreams-metrics/daily/$(ts_date).json"
...
# The sweet-dreams cron job is killed externally by the gateway on timeout (SIGTERM
...
DREAM_LOCK="$OPENCLAW_WORKSPACE/logs/sweet-dreams.lock"
...
main() {
  ...
  stage "Trigger check"
  log INFO "Sweet Dreams is managed as an OpenClaw cron job."
  log INFO "Trigger: use 'openclaw cron run total-recall-sweet-dreams' or wait for 5 AM schedule."
  ...
  log INFO "  1. Run: openclaw cron run total-recall-sweet-dreams (or via cron tool)"
