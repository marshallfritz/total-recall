#!/usr/bin/env bash
# Sweet Dreams helper script (Phase 1 MVP)
# Called by the nightly Sweet Dreams agent for safe file operations.
...
METRICS_DIR="$OPENCLAW_WORKSPACE/research/sweet-dreams-metrics/daily"
...
DREAM_LOCK_FILE="$OPENCLAW_WORKSPACE/logs/sweet-dreams.lock"
...
cmd_preflight() {
  ...
  # Abort if a Sweet Dreams is already running (stale lock auto-cleared via expiry field)
  if [ "$dry_run" = "false" ] && [ -f "$DREAM_LOCK_FILE" ]; then
    if lock_is_expired "$DREAM_LOCK_FILE"; then
      ...
      error "Sweet Dreams already running (lock expires: ${expires}). Aborting to prevent race condition."
      exit 1
    fi
  fi
  ...
  if [ "$dry_run" = "false" ]; then
    write_lock
    info "Sweet-dreams lock written: $DREAM_LOCK_FILE (expires $(cut -d: -f3 "$DREAM_LOCK_FILE"))"
  fi
...
cmd_archive() {
  ...
  [ -n "$archive_file" ] || { err "Usage: sweet-dreams.sh archive <archive-file> <json-data?>"; exit 1; }
  ensure_dirs
...

  {
    ...
    echo
    echo "Archived by Sweet Dreams nightly run."
    echo
    echo "---"
    echo
...
cmd_chunk() {
  ...
  [ -n "$chunk_file" ] || { err "Usage: sweet-dreams.sh chunk <chunk-file> <json-data?>"; exit 1; }
  ensure_dirs
...

  {
    if [ -f "$chunk_path" ]; then
      ...
      echo "Chunked by Sweet Dreams Phase 2 (Wisdom Builder)."
      echo
      echo "---"
      echo
      printf '%s' "$chunk_entry"
...
cmd_update_observations() {
  local new_file="${1:-}"
  [ -n "$new_file" ] || { err "Usage: sweet-dreams.sh update-observations <new-observations-file>"; exit 1; }
...
cmd_write_log() {
  local log_file="${1:-}"
  local json_arg="${2:-}"
  [ -n "$log_file" ] || { err "Usage: sweet-dreams.sh write-log <log-file> <json-data?>"; exit 1; }
...
cmd_write_metrics() {
  local json_file="${1:-}"
  local json_arg="${2:-}"
  [ -n "$json_file" ] || { err "Usage: sweet-dreams.sh write-metrics <json-file> <json-data?>"; exit 1; }
...
cmd_write_staging() {
  ...
  [ -n "$staging_file" ] || { err "Usage: sweet-dreams.sh write-staging <staging-file> <json-data?>"; exit 1; }
  ensure_dirs
...
cmd_validate() {
  ...
  # Release sweet-dreams lock on clean validation
  rm -f "$DREAM_LOCK_FILE"
...
cmd_log_stage() {
  ...
  local log_file="$OPENCLAW_WORKSPACE/logs/sweet-dreams-run.log"
...
cmd_rollback() {
  ...
  # Release sweet-dreams lock on rollback (failed run)
  rm -f "$DREAM_LOCK_FILE"
...
usage() {
  cat <<'EOF'
Usage:
  sweet-dreams.sh preflight [--dry-run]
  sweet-dreams.sh archive <archive-file> <json-data?>
  sweet-dreams.sh chunk <chunk-file> <json-data?>
  sweet-dreams.sh decay
  sweet-dreams.sh update-observations <new-observations-file>
  sweet-dreams.sh write-log <log-file> <json-data?>
  sweet-dreams.sh write-metrics <json-file> <json-data?>
  sweet-dreams.sh write-staging <staging-file> <json-data?>
  sweet-dreams.sh log-stage <stage-name> [message]
  sweet-dreams.sh validate
  sweet-dreams.sh rollback
