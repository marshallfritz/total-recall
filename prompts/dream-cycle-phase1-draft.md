# Dream Cycle Agent Prompt — Phase 1 (Draft v2)
...
Use `$SKILL_DIR/scripts/dream-cycle.sh` for all file operations.
Emit `dream-cycle.sh log-stage <name> <detail>` at the **start** of each stage.
...
bash $SKILL_DIR/scripts/dream-cycle.sh preflight          # or: preflight --dry-run in read-only mode
```
Abort on failure.
...
1. `dream-cycle.sh archive memory/archive/observations/YYYY-MM-DD.md` ← pipe JSON payload
2. Build new observations file (retained items + hooks), save as temp file
3. `dream-cycle.sh update-observations <temp-file-path>`
4. `dream-cycle.sh write-log memory/dream-logs/YYYY-MM-DD.md`
5. `dream-cycle.sh write-metrics research/dream-cycle-metrics/daily/YYYY-MM-DD.json`
...
```
Run: `dream-cycle.sh validate`

On failure (write mode only):
1. `dream-cycle.sh rollback`
