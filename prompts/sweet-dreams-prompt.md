# Sweet Dreams Agent Prompt (Phase 1 MVP)
...
You must use `$SKILL_DIR/scripts/sweet-dreams.sh` (where `SKILL_DIR` is the total-recall skill directory, e.g. `~/your-workspace/skills/total-recall`) for file operations.

---

## Required Sequence

> **Observability:** Call `sweet-dreams.sh log-stage <name> [detail]` at the start of each stage below.
> This writes a timestamped line to `logs/sweet-dreams-run.log` — the tailable live log.
...
```
- If read-only:
  - `bash $SKILL_DIR/scripts/sweet-dreams.sh preflight --dry-run`
- Otherwise:
  - `bash $SKILL_DIR/scripts/sweet-dreams.sh preflight`
...
> **WP2 note:** If `DREAM_PHASE >= 2`, run `sweet-dreams.sh decay` **before** classification to apply daily importance decay to existing scores in `observations.md`.
...
4. Build the proposal JSON payload:
   ...
   ```
5. Write the proposal via: `sweet-dreams.sh write-staging memory/dream-staging/YYYYMMDD-HHMMSS-[type].md '<json>'`
...
2. For each valid cluster (3+ items):
   ...
   b. Write chunk: `sweet-dreams.sh chunk memory/archive/chunks/YYYY-MM-DD.md '{"id":"CHUNK-...","topic":"...","date_range_start":"...","date_range_end":"...","confidence":"...","source_ids":["..."],"finding":"..."}'`
...
Archived by Sweet Dreams nightly run.
---
## OBS-YYYYMMDD-001
**Original date**: [date]
**Impact**: [level]
...
**Rationale:** Each Sweet Dreams run appends new hooks without removing old ones. Without a cap, the hooks section accumulates unboundedly and becomes the dominant source of file bloat. At 20 hooks × ~350 bytes each, the section stays ≤ 7KB.
...
1. Write archive file:
   ...
   - `sweet-dreams.sh archive memory/archive/observations/YYYY-MM-DD.md`
2. Build a new observations file with retained items + hooks, save temp file in workspace.
3. Apply update atomically:
   - `sweet-dreams.sh update-observations <temp-file-path>`
4. Write dream log:
   - `sweet-dreams.sh write-log memory/dream-logs/YYYY-MM-DD.md`
5. Write metrics JSON:
   - `sweet-dreams.sh write-metrics research/sweet-dreams-metrics/daily/YYYY-MM-DD.json`
...
```
Run:
- `sweet-dreams.sh validate`

If validation fails in write mode:
1. Run `sweet-dreams.sh rollback`
...
- Pattern promotions go to `memory/dream-staging/` only via `sweet-dreams.sh write-staging`. Never direct writes.
...
This table must appear in both the dream log file (written via `sweet-dreams.sh write-log`) and in the inline execution summary.
